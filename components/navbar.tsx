                      ...
                      <Link href={item.href} key={item.href} onClick={() => setIsOpen(false)}>
                        <div
                          className={cn(
                            ...
                            isActive ? "bg-gray-100 text-primary font-medium" : "text-gray-600 hover:bg-gray-50",
                          )}
                        >
                          <Icon className={cn("mr-3 h-5 w-5", isActive ? "text-primary" : "text-gray-600")} />
                          {item.label}
                        </div>
                      </Link>
                    )
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    key={item.href}
                    className={cn(
                      "text-lg py-6 px-6 flex items-center",
                      isActive
                        ? "border-primary text-primary border-b-2"
                        : "text-gray-600 hover:text-primary hover:bg-gray-50",
                    )}
                  >
                    <Icon className={cn("mr-2 h-5 w-5", isActive ? "text-primary" : "text-gray-600")} />
                    {item.label}
                  </Link>
                )
              })}

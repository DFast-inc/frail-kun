...
export async function PUT(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    return NextResponse.json({ success: true, lastUpdated: session.lastUpdated })
  } catch (error) {
    console.error("検査セッション更新エラー:", error)
    return NextResponse.json({ error: "検査セッションの更新に失敗しました" }, { status: 500 })
  }
}
...
export async function POST(request: Request, { params }: { params: { sessionId: string } }) {
  try {
    ...
    return NextResponse.json({ error: "検査データの保存に失敗しました" }, { status: 500 })
  }
}

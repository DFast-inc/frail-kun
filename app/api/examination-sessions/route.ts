...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "検査セッションの作成に失敗しました" }, { status: 500 })
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!sessionId) {
      ...
      const activeSessions = Object.values(examinationSessions).filter((session: any) => session.status === "active")
    ...
    return NextResponse.json({ error: "検査セッションの取得に失敗しました" }, { status: 500 })
  }
}
